...
//modified parts
  ...
  for (int i=0; i<noterow.length; i++){
    noterow[i] = new NoteButton(30+i*45, 290, 30, 30);
    noterow2[i] = new NoteButton(30+i*45, 360, 30, 30);
  }
...
void draw(){
  ...
  fill(0);
  rect(470, 310, 530, height);//synth area
...
//FROM SYNTHESIZE SOUND EXAMPLE -------------------------------------
  ...
  for(int i = 0; i < out.bufferSize() - 1; i++){
    //offset sine wave by 470
    line( i+470, 390  - out.left.get(i)*50,  i+471, 390  - out.left.get(i+1)*50 );
    line( i+470, 490 - out.right.get(i)*50, i+471, 490 - out.right.get(i+1)*50 );
  }
  
  
  stroke(255);
  fill(100); 
  rect(430, 310, 20, 240);//volume slider
  fill(0,200,200);
  rect(430, slideY, 20, 10);
...
//UI descriptions
  ...
  textAlign(CORNER);
  text("Drum kit", 20, 30);
  text("Chord Presets (C major triads)", 30, 280);
  text("Spicier chords", 30, 350);
...
//FROM DRUM KIT EXAMPLE ------------------------------------------------
 ...
 //C major chord labels (rough estimate)
  textAlign(CORNER);
  fill(0);
  text("C        Dm       Em        F         G        Am     Bdim       C", 40, 310);
  text("C7      Dm7    Em7     FM7     GM7     Am7    Bm7      C7", 37, 380);
}
...
//FROM DRUM KIT EXAMPLE ------------------------------------------------  
  ...
  for(int i=0; i < noterow.length; ++i)
  noterow[i].mousePressed(); //call noterow's mousePressed() function in other tab
...
//FROM SYNTHESIZE SOUND EXAMPLE -------------------------------------  
  ...
  if (mousePos.y >= 310 && mousePos.x >=470){ //can only affect the sine wave when within this area
  
 //the range of possible mouseX values corresponds to numbers between 110 and 880: 
  float freq = map( mouseX, 470, width, 40, 880 );
